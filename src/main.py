...
parser.add_argument("--restrict-movement", action="store_true", help="Force all walls to restrict movement (now the default; kept for compatibility)")
parser.add_argument("--no-restrict-movement", action="store_true", help="Emit walls that do not block movement, as Roll20 pages with dynamic lighting movement turned off behaved")
...
parser.add_argument("--auto-doors", action="store_true", help="Detect doors from wall colours. Now the default on pages that carry no door objects; kept for compatibility.")
parser.add_argument("--no-auto-doors", action="store_true", help="Never infer doors from wall colours, even on a legacy page that encodes them that way")
