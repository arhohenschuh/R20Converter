...
| [B056](B056-asset-extension-not-renderable.md) | High | Fixed | The stored extension came from the Roll20 URL, not the content. A cache-buster after `&` survived (`….svg&cb=5`, 52 assets), and `.jfif` is absent from Foundry's `IMAGE_FILE_EXTENSIONS`, so the *Lakeside* map converted to the correct path and was never drawn. Every existing check passed — the file existed, was non-empty and resolved; nothing asked whether the client could render it. |
